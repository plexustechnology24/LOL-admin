...
import { faArrowUpFromBracket, faChevronDown, faCopy, faEdit, faEye, faFileImage, faPlus, faTrash } from "@fortawesome/free-solid-svg-icons";
...
const EmotionEmoji = () => {
    ...
    const [imageFileLabel, setImageFileLabel] = useState('Emotion Emoji Upload');
    ...
    const getData = useCallback((page = null) => {
        ...
        axios.post('https://api.lolcards.link/api/emotion/emoji/read', payload)
            .then((res) => {
    ...
    const handleDelete = () => {
        axios
            .delete(`https://api.lolcards.link/api/emotion/emoji/delete/${deleteModal.id}`)
            .then((res) => {
    ...
    const toggleModal = (mode) => {
        if (!visible) {
            if (mode === 'add') {
                setId(undefined);
                setImageFileLabel('Emotion Emoji Upload');
                setSelectedFiles([]);
                setPreviewUrls([]);
                setExistingImageUrl('');
                setCategory('');
    ...
    const handleFileChange = (event) => {
        ...
        for (const file of files) {
            ...
            // Validate file type (allow all image types including SVG)
        ...
        try {
            ...
            if (id) {
                if (selectedFiles.length > 0) {
                    ...
                    formData.append('Emoji', selectedFiles[0]);
                    formData.append('Category', Category);

                    const response = await axios.patch(
                        `https://api.lolcards.link/api/emotion/emoji/update/${id}`,
                        formData,
                        {
                            headers: {
                ...
                for (const file of selectedFiles) {
                    ...
                    formData.append('Category', Category);
                    uploadPromises.push(
                        axios.post('https://api.lolcards.link/api/emotion/emoji/create', formData, {
                            headers: {
                                'Content-Type': 'multipart/form-data'
                            }
                        })
    ...
    const handleEdit = (emoji) => {
        setExistingImageUrl(emoji.Emoji);
        setPreviewUrls([emoji.Emoji]);
        setSelectedFiles([]);
        setCategory(emoji.Category || '');
        setId(emoji._id);
        setImageFileLabel('Emotion Emoji Upload (Select new file to replace, or keep existing)');
        setVisible(true);
    };
    ...
    return (
        <div>
            <PageBreadcrumb pageTitle="Emotion Emoji" />
                    ...
                    <div className="px-6 pt-5">
                        <div className="flex justify-between items-center px-4 py-3 mt-4 dark:border-gray-800 border-gray-200 gap-4">
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    <FontAwesomeIcon icon={faPlus} className='pe-2' /> Add Emoji
                                </Button>
                            </div>
                        </div>
                    </div>
                            ...
                            <Table>
                                <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                                    <TableRow>
                                        ...
                                        <TableCell isHeader className="py-7 font-medium text-gray-500 px-2 border-r border-gray-200 dark:border-gray-700">Emotion Emoji</TableCell>
                                            ...
                                            <TableRow key={emoji._id}>
                                                <TableCell className="py-3 px-2 border-r border-gray-200 dark:border-gray-700">
                                                    <div className="flex items-center justify-center">
                                                        <input
                                                            type="checkbox"
                                                            className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700"
                                                            checked={selectedItems.includes(emoji._id)}
                                                            onChange={() => handleSelectItem(emoji._id)}
                                                        />
                                                    </div>
                                                </TableCell>
                                                ...
                                                <TableCell className="py-3 px-2 border-r border-gray-200 dark:border-gray-700">
                                                    <div className="flex align-middle justify-center gap-4 h-full">
                                                        <button style={{ color: "#0385C3" }} onClick={() => handleEdit(emoji)}>
                                                            <FontAwesomeIcon icon={faEdit} className="text-lg" />
                                                        </button>
                                                        <button className="text-red-600" onClick={() => openDeleteModal(emoji._id)}>
                    ...
                    <div className="relative bg-white rounded-lg w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
                        <div className="px-6 py-4 border-b">
                            <h3 className="text-xl font-semibold">
                                {id ? "Edit Emotion Emoji" : "Add Emotion Emoji"}
                            </h3>
                        </div>
                        ...
                        <div className="px-6 py-4">
                            <form onSubmit={handleSubmit}>
                                <div className="py-2">
                                    ...
                                    <label className="block font-medium mb-2">
                                        {imageFileLabel} <span className="text-xs">(Max {MAX_FILES} images, 5 MB each, supports JPG, PNG, GIF, WEBP, SVG)</span>
                                        <span className="text-red-500 pl-2 font-normal text-lg">*</span>
                                    </label>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[99999]">
                    <div className="bg-white rounded-lg p-6 w-full max-w-sm mx-4 shadow-lg">
                        ...
                        <h2 className="text-xl font-semibold text-gray-800 mb-3">
                            {deleteModal.isBulk ? 'Delete Selected Items' : 'Delete Emotion Emoji'}
                        </h2>

                        <p className="text-gray-700 mb-6">
                            {deleteModal.isBulk
                                ? `Are you sure you want to delete ${selectedItems.length} selected items?`
                                : 'Are you sure you want to delete this Emotion Emoji?'
                            }
                        </p>
            ...
            <ImagePreviewModal
                ...
                images={currentItems.map(item => item.Emoji)} // Only use current page items
