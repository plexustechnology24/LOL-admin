...
const BluffCardBg = () => {
    ...
    const getData = useCallback((page = null) => {
        ...
        axios.post('https://api.lolcards.link/api/bluff/read', payload)
            .then((res) => {
    ...
    const handleDelete = () => {
        axios
            .delete(`https://api.lolcards.link/api/bluff/delete/${deleteModal.id}`)
            .then((res) => {
            ...
            if (id) {
                if (selectedFiles.length > 0) {
                    ...
                    const response = await axios.patch(
                        `https://api.lolcards.link/api/bluff/update/${id}`,
                        formData
                    );
                ...
                for (const file of selectedFiles) {
                    ...
                    formData.append('Category', Category);
                    uploadPromises.push(
                        axios.post('https://api.lolcards.link/api/bluff/create', formData)
                    );
                }
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (setIsAccessOpen && !event.target.closest('.category-filter-dropdown')) {
    ...
    }, [setIsAccessOpen, isCategoryDropdownOpen]);
    ...
    return (
        <div>
            <PageBreadcrumb pageTitle="Bluff CardBg" />
                    ...
                    <div className="px-6 pt-5">
                        <div className="flex justify-between items-center px-4 py-3 mt-4 dark:border-gray-800 border-gray-200 gap-4">
                            ...
                            <div className="flex gap-3">
                                {/* Category Filter */}
                                <div className="relative inline-block w-64 category-filter-dropdown">
                                    <button
                                            ...
                                            <TableRow key={cardbg._id}>
                                                ...
                                                <TableCell className="py-3 px-2 border-r border-gray-200 dark:border-gray-700">
                                                    <div className="flex items-center justify-center gap-2">
                                                        {cardbg.Category && categories.find(cat => cat.id === cardbg.Category) ? (
                                                            <>
                                                                <img
                                                                    src={categories.find(cat => cat.id === cardbg.Category)?.image}
                                                                    alt={cardbg.Category}
                                                                    className="w-6 h-6 object-contain"
                                                                />
                                                                <span className="dark:text-gray-400">{cardbg.Category}</span>
                                                            </>
                                                        ) : (
